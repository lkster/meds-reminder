...
M27 adds the deterministic normal-app Light/Dark semantic theme foundation, a dedicated Settings
screen, and a dedicated Alarm Readiness screen. Existing sound, vibration, Snooze, and Android
capability behavior is presented there without changing Room, scheduling, or alarm delivery.
Medication, History, and editor layouts inherit the theme foundation; their full M26 redesigns
remain deferred. The M15 Samsung Galaxy S23 validation below remains historical evidence and was
not repeated for this presentation-only milestone.
...
Medication-card headers now reserve horizontal space for the native enable/disable Switch. Valid long
medication names wrap in the remaining width instead of competing with the Switch's layout allocation.
The native Switch, M13 semantics, and stable-ID mutation behavior are unchanged; no Room, alarm,
lifecycle, persistence, or navigation behavior changed.
...
content remains centered. M14 loading/null handling and Room queue ownership remain unchanged; no
alarm delivery, scheduling, persistence, task, service, or Direct Boot behavior changed.
...
reminder-row position and time, including full weekday names for accessibility services while the
visible abbreviated weekday labels remain unchanged. These are presentation-only semantics: no
Room, saved-state, lifecycle, alarm, or scheduling ownership changed.
